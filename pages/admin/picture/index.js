...
import columns from '@/core/components/picture/columns';
import PictureForm from '@/core/components/picture/pictureForm';
...
const Index = () => {
  ...
  const fetchData = async (params = {}) => {
    ...
    try {
      ...
      const sortQuery = sortField ? `&sortField=${sortField}&sortOrder=${sortOrder}` : '';
      
      const filterQuery = filters
        ? filters.map((filter, index) => `&filterField_${index}=${filter.field}&filterValue_${index}=${filter.value}`).join('')
        : '';
        
      const response = await fetch(`/api/picture?page=${pagination?.current || 1}&pageSize=${pagination?.pageSize || 10}${sortQuery}${filterQuery}`);
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    const filterEntries = Object.entries(filters).reduce((acc, [field, value]) => {
  ...
  const confirmPopconfirm = async (record) => {
    try {
      const response = await fetch(`/api/picture?id=${record.id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
