...
import columns from '@/core/components/stock/columns';
import StockForm from '@/core/components/stock/stockForm';
...
const Index = () => {
  ...
  const fetchData = async (params = {}) => {
    ...
    try {
      ...
      const response = await fetch(`/api/stock?page=${pagination?.current || 1}&pageSize=${pagination?.pageSize || 10}${sortQuery}${filterQuery}`);
  ...
  const confirmPopconfirm = async (record) => {
    try {
      const response = await fetch(`/api/stock?id=${record.id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
