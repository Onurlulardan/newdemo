...
import columns from '@/core/components/user/columns';
...
const Index = () => {
  ...
  const fetchData = async (params = {}) => {
    ...
    try {
      ...
      const response = await fetch(`/api/user?page=${pagination?.current || 1}&pageSize=${pagination?.pageSize || 10}${sortQuery}${filterQuery}`);
  ...
  const confirmPopconfirm = async (record) => {
    try {
      const response = await fetch(`/api/user?id=${record.id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
