...
import columns from '@/core/components/product/columns';
import ProductForm from '@/core/components/product/productForm';
...
const Index = () => {
  ...
  const fetchData = async (params = {}) => {
    ...
    try {
      ...
      const response = await fetch(`/api/product?page=${pagination?.current || 1}&pageSize=${pagination?.pageSize || 10}${sortQuery}${filterQuery}`);
  ...
  const confirmPopconfirm = async (record) => {
    try {
      const response = await fetch(`/api/product?id=${record.id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
