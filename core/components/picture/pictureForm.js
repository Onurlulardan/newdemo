...
import { Form, Input, InputNumber, Select, Upload, Button, message, Checkbox, DatePicker } from 'antd';
...
const PictureForm = ({ record, onClose, fetchData }) => {
        ...
        const fetchRelationData = async () => {
            ...
            try {
                ...
                const columns = [
                            ...
                            { dataIndex: 'productId', relationModelKey: 'name' },
                ...
                await Promise.all(
                    responses.map(async (response, index) => {
                        if (response.ok) {
                            ...
                            message.error(`An error occurred while loading related data: ${response.statusText}`);
                        }
                    })
                );
    ...
    return (
        <Form form={form} layout="vertical" onFinish={onFinish} initialValues={record || initialValues} key={record ? record.id : 'new'}>
                    ...
                    >
                            ...
                            <Dragger {...uploadProps}>
                                ...
                                <p className="ant-upload-hint">
                                    Only one file can be uploaded. Supported types: .png, .jpg, .jpeg, .pdf, etc.
                                </p>
                            </Dragger>
                    ...
                    >
                            ...
                            <Select placeholder="Select product" allowClear>
                                {(relationOptions['productId'] || []).map(option => (
                                    <Select.Option key={option.value} value={option.value} loading={isLoading}>
                                        {option.label}
                                    </Select.Option>
                                ))}
                            </Select>
